...
- 프론트엔드와 백엔드를 **하나의 흐름으로 연결하는 개발**을 선호합니다
- 사용자 경험과 데이터 흐름을 함께 고려하는 개발자가 되고자 합니다
...
프로젝트를 진행하며 발생한 주요 오류의 원인을 분석하고 해결한 경험입니다.

### 🎱 CueLinkProject

* **JWT 인증 오류 해결**
  토큰 변수가 잘못 매핑되어 `jwt malformed` 오류가 발생했습니다. 클라이언트와 인증 미들웨어에서 사용하는 토큰 전달 변수를 동일하게 수정했습니다.

* **Node.js 서버 실행 경로 오류 해결**
  `package.json`의 실행 경로가 존재하지 않는 `index.js`로 설정되어 서버가 종료되었습니다. 실행 진입점을 `src/server.js`로 변경했습니다.

* **Oracle CLOB 순환 참조 오류 해결**
  게시글 본문을 CLOB 객체 상태로 JSON 응답하면서 순환 참조 오류가 발생했습니다. SQL 조회 과정에서 `TO_CHAR`를 적용해 문자열로 변환했습니다.

* **카테고리 ID 타입 오류 해결**
  카테고리 ID가 `undefined` 또는 문자열로 전달되어 Oracle 숫자 바인딩 오류가 발생했습니다. API 호출 경로를 수정하고 ID를 숫자로 명시적 형변환했습니다.

* **Oracle 바인드 매핑 오류 해결**
  SQL문의 바인드 변수 개수와 전달된 데이터 개수가 일치하지 않아 오류가 발생했습니다. SQL문과 바인드 객체를 비교해 누락된 값을 추가했습니다.

* **환경변수 DB 연결 오류 해결**
  `.env` 파일의 키와 서버에서 참조하는 환경변수명이 일치하지 않아 Oracle 연결 문자열을 불러오지 못했습니다. 환경변수 키를 통일해 해결했습니다.
...
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,vue,react,flutter,dart&theme=dark" />
</p>
...
<p>
  <img src="https://skillicons.dev/icons?i=git,github,figma,aws,vscode,eclipse&theme=dark" />
</p>
...
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/leetaehwa1)
[![Email](https://img.shields.io/badge/Email-0078D4?style=for-the-badge&logo=gmail&logoColor=white)](mailto:th3926@naver.com)
